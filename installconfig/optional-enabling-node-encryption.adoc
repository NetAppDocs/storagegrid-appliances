...
Additionally, when you enable FIPS mode after enabling node encryption, the NetApp StorageGRID Kernel Crypto API 6.1.129-1-ntap1-amd64 module is used for encryption of data at rest. Refer to https://docs.netapp.com/us-en/storagegrid/admin/manage-tls-ssh-policy.html#select-a-security-policy[Select a security policy^] for more information.
