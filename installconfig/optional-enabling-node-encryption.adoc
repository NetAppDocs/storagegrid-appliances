...
Additionally, when you enable FIPS mode after enabling node encryption, it provides FIPS certified encryption of data at rest. Refer to https://review.docs.netapp.com/us-en/storagegrid_storagegrid-120-dev/admin/manage-tls-ssh-policy.html#select-a-security-policy[Select a security policy^] for more information.
...
If you have a StorageGRID appliance with FIPS drives, enabling Drive Encryption also provides FIPS certified encryption of data at rest.
