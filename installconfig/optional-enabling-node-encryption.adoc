...
---
= Enable node or drive encryption for StorageGRID appliances
:icons: font
:imagesdir: ../media/
...
Additionally, when you enable FIPS mode after enabling node encryption, the NetApp StorageGRID Kernel Crypto API 6.1.129-1-ntap1-amd64 module is used for encryption of data at rest. Refer to https://review.docs.netapp.com/us-en/storagegrid_storagegrid-120-dev/admin/manage-tls-ssh-policy.html#select-a-security-policy[Select a security policy^] for more information.
...
Drive encryption can be used for any SED SSD installed in an SG100, SG1000, SG110, SG1100, SG120, SG1200, SGF6112, SGF6212, SG6100-CN, or SG6200-CN compute node or controller.

* For services appliances, the SSDs are the node root disks.
* In an SG6100-CN or SG6200-CN controller, the SSDs are used for caching.
* In an SGF6112 or SGF6212, the SSDs are the node root disks and are used for the primary storage of object data.
