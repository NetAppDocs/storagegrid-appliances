...
. Continue monitoring the installation progress until the *Install StorageGRID* stage pauses and a message appears on the embedded console, prompting you to approve this node on the Admin Node using the Grid Manager; then, go to the next step.

. https://docs.netapp.com/us-en/storagegrid/admin/signing-in-to-grid-manager.html[Go to the Grid Manager^] of the Primary Admin node, approve the pending storage node, and complete the StorageGRID installation process.
+
