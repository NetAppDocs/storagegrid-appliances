...
---
= Cable a StorageGRID appliance or controller
:icons: font
:imagesdir: ../media/
...
* You have one of the following options for the network ports. These items aren't provided with the appliance.
 ** One to four TwinAx cables for connecting the four network ports.
 ** For the SG120, one to four SFP+ or SFP28 transceivers if you plan to use optical cables for the ports.
 ** For the SG1200, one to four QSFP+, QSFP28, or QSFP56 (200GbE NIC only) transceivers if you plan to use optical cables for the ports.
...
SG120 port connections:

image::../media/sg120_connections.png["SG120 rear connectors"] 

SG1200 port connections:

image::../media/sg1200_connections.png["SG1200 port connections"] 
...
* 10/25/40/100-GbE with 100G NIC
* 10/25/50/100/200-GbE (200GbE NIC only)

|Connect to the Grid Network and the Client Network for StorageGRID. See link:gathering-installation-information-sg120-and-sg1200.html#port-bond-modes[Port bond modes (SG120 and SG1200)]
...
[cols="2a,2a" options="header"]
|===
| SG120 link speed (GbE)| Required equipment
|10
|SFP+ transceiver
...
[cols="2a,2a" options="header"]
|===
| SG1200 link speed (GbE) | Required equipment
|10
|QSA and SFP+ transceiver
...
+
NOTE: The physical link state for port 1 is unavailable in software and must be verified at this time using the status LED on the SG5800 controller.
...
* SG6200 only: You have the 100GbE to 4x25GbE breakout cable provided with the appliance for connecting the two storage controllers to the SG6200-CN controller.
* You have RJ-45 Ethernet cables:
** One RJ-45 cable for connecting the management port.
** SG6200 only: Up to four additional RJ-45 Ethernet cables for the optional ports you plan to use, including the second Admin port and the BMC port on the SG6200-CN, and the maintenance ports on each of the two E4000 controllers.
* You have one of the following options for the network ports. These items aren't provided with the appliance.
 ** One to four TwinAx cables for connecting the four network ports.
 ** One to eight SFP+, SFP28, QSFP+, QSFP28, or QSFP56 transceivers if you plan to use optical cables for the ports.

CAUTION: *Risk of exposure to laser radiation* -- Don't disassemble or remove any part of an SFP transceiver. You might be exposed to laser radiation.

.About this task

The following figures show the ports on the back of the SGF6212 and the three controllers in the SG6200 appliance. The SG6200 appliance includes a SG6200-CN compute controller on top and two E4000 storage controllers on the bottom.


SGF6212 connections:

image::../media/sgf6212_connections.png["SGF6212 rear connectors"] 

SG6260 connections:

image::../media/sg6200_e4000_connections.png["SG6200-CN to E4000 Connections"] 
...
|2
|Four network ports on the SG6200-CN controller
|
* Four 10/25/40/100-GbE network ports or 200-GbE with optional 200G NIC SKU
|Connect to the Grid Network and the Client Network for StorageGRID. See link:gathering-installation-information-sg6200.html#port-bond-modes[Port bond modes (SG6200)]
...
|Five total connection ports
|
* One 100GbE port on the SG6200-CN
* Two 10/25GbE ports on each of the storage controllers
|Connect each storage controller to the SG6200-CN controller.
...
+
Although this connection is optional, it is recommended to facilitate support.
. Connect the two interconnect ports on each storage controller to the 100GbE port on the SG6200-CN controller, using one 100-GbE QSFP28 to 4x25-GbE SFP28 cable.
. Connect the network ports on the appliance to the appropriate network switches, using TwinAx cables or optical cables and transceivers.
+
[cols="2a,2a" options="header"]
|===
...
. If your appliance includes an expansion shelf, see the link:../sg6200/adding-expansion-shelf-to-deployed-sg6260.html[instructions for adding an expansion shelf to a deployed SG6200] for cabling information.
--
