...
You connect the two controllers to each other, connect the management ports on each controller, and connect the 10/25-GbE ports on the E5800SG controller to the Grid Network and optional Client Network for StorageGRID.
 ...
 ** Two power cords.
 ** Two optical cables for the FC interconnect ports on the controllers.
 ** Eight SFP+ transceivers, which support either 10-GbE or 16-Gbps FC. The transceivers can be used with the two interconnect ports on both controllers and with the four 10/25-GbE network ports on the E5800SG controller, assuming you want the network ports to use a 10-GbE link speed.
* You have obtained the following items, which aren't included with the appliance:
 ** One to four optical cables for the 10/25-GbE ports you plan to use.
 ** One to four SFP28 transceivers, if you plan to use 25-GbE link speed.
 ** Ethernet cables for connecting the management ports.
...
The figures show the two controllers in the SG5860, with the E4000 series storage controller on the top and the E5800SG controller on the bottom. In the SG5812, the E4000 series storage controller is to the left of the E5800SG controller when viewed from the back.
...
3
a|
Management port 1 on the E5800SG controller
a|
1-GbE (RJ-45)
a|
Connects the E5800SG controller to the Admin Network for StorageGRID.
a|
3
a|
Management port 2 on the E5800SG controller
a|
1-GbE (RJ-45)
a|
...
* Can be bonded with management port 1 if you want a redundant connection to the Admin Network.
* Can be left unwired and available for temporary local access (IP 169.254.0.1).
* During installation, can be used to connect the E5800SG controller to a service laptop if DHCP-assigned IP addresses aren't available.

a|
4
a|
10/25-GbE ports 1-4 on the E5800SG controller
a|
10-GbE or 25-GbE
...
a|
Connect to the Grid Network and the Client Network for StorageGRID. See link:gathering-installation-information-sg5700.html#port-bond-modes[Port bond modes (E5800SG controller)].
|===

.Steps

. Connect the E4000 controller to the E5800SG controller, using two optical cables and four of the eight SFP+ transceivers.
+
[cols="1a,1a" options="header"]
|===
| Connect this port...| To this port...
...
a|
Interconnect port 1 on the E5800SG controller
a|
Interconnect port 2 on the E4000 controller
a|
Interconnect port 2 on the E5800SG controller
|===
...
+
Don't use management port 2 (P2) on the E4000 controller (the RJ-45 port on the right). This port is reserved for technical support.

. If you plan to use the Admin Network for StorageGRID, connect management port 1 on the E5800SG controller (the RJ-45 port on the left) to the Admin Network, using an Ethernet cable.
+
If you plan to use active-backup network bond mode for the Admin Network, connect management port 2 on the E5800SG controller (the RJ-45 port on the right) to the Admin Network, using an Ethernet cable.

. Connect the 10/25-GbE ports on the E5800SG controller to the appropriate network switches, using optical cables and SFP+ or SFP28 transceivers.
+
NOTE: Install SFP+ transceivers if you plan to use 10-GbE link speeds. Install SFP28 transceivers if you plan to use 25-GbE link speeds.
...
image::../media/sg6100_e4000_connections.png[SG6160 to E4000 Connections]
...
|2
|
SGF6112: Four 10/25-GbE network ports on the appliance

SG6160: Eight total 10/25 GbE ports

* Four on the SG6100-CN controller
* Two on each storage controller
| 
...
|10/25-GbE
|Connect to the Grid Network and the Client Network for StorageGRID. See link:../installconfig/gathering-installation-information-sg6100.html#port-bond-modes[Port bond modes (SG6100-CN controller)].
