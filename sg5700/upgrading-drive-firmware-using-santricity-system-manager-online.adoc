...
[NOTE]
====
Drives belonging to volumes that don't have redundancy must be updated using the link:upgrading-drive-firmware-using-santricity-system-manager-offline[offline method]. The offline method should be used for any drive associated with flash read cache, or any pool or volume group that is currently degraded. 

You must use the link:upgrading-drive-firmware-using-santricity-system-manager-offline[offline method] when upgrading SSD drives.
====
