...
=== Step 2c: Remove the mezzanine card

Remove the mezzanine bezel and PCIe mezzanine card from the impaired controller canister.

.Steps

. Remove the mezzanine card bezel by sliding it straight out from the controller canister.
...
=== Step 2d: Move the boot media

Remove the boot media from the impaired controller canister and install it in the replacement controller canister.

.Steps

. After removing the mezzanine card, locate the boot media using the following illustration or the FRU map on the controller canister:
...
=== Step 2e: Install the mezzanine card in the replacement controller
Install the mezzanine card in the replacement controller canister.
...
|DIMMS
|===
. Verify that the NVMEM battery is not plugged into the replacement controller canister.
...
=== Step 2g: Install the battery

Install the NV battery into the replacement controller canister.
...
Install the replacement controller canister into the system chassis and boot ONTAP.
