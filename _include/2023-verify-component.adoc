...
* You have the serial number of the appliance where the component needs to be replaced.
...
Use this procedure to identify the appliance with failed hardware and which of the replaceable hardware components isn't operating properly. Components that might be identified for replacement can include: 
...
.. From the table on the Nodes page, select the StorageGRID appliance name with the failed component.
 ...
 ** If the StorageGRID Appliance section in Grid Manager does not display, the node selected isn't a StorageGRID appliance. Select a different node from the tree view.
