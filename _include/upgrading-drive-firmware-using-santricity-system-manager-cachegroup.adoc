//This include file is used for the SG6000. SG5700 and all storage appliances after 11.6 use the common file.
.Before you begin
...
+
NOTE: While the appliance is in maintenance mode, I/O (input/output operations) activity to the storage controller is stopped to make disruptive storage operations safe.
...
CAUTION: The SG6060 contains two types of drives: SSD and HDD. You must use the offline method to upgrade the firmware on the SSDs. You can use either the online or offline method to upgrade the firmware on the HDDs.
...
. If you are upgrading SSD drives that are part of an SSD cache group, suspend the cache group:
 .. From SANtricity System Manager, select *Storage* > *POOLS & VOLUME GROUPS*.
 .. Select the SSD cache group to suspend.
 .. Select *More* > *Suspend/Resume SSD Cache*.
...
. If you upgraded SSD drives that were part of an SSD cache group, the cache group must be resumed:
 .. From SANtricity System Manager, select *Storage* > *POOLS & VOLUME GROUPS*.
 .. Select the SSD cache group to resume.
 .. Select *More* > *Suspend/Resume SSD Cache*.
