...
+
NOTE: IP configuration changes made while the appliance is in maintenance mode are not applied to the installed StorageGRID environment. Run the link:../maintain/changing-nodes-network-configuration.html[`change-ip` command] after rebooting the appliance into StorageGRID.
