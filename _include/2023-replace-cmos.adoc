...
. If the appliance you replaced had drive encryption enabled for the SED drives, you must link:optional-enabling-node-encryption.html#access-an-encrypted-drive[enter the drive encryption passphrase] to access the encrypted drives when the replacement appliance starts for the first time.
...
. Log in to the appliance:
  ...
  .. Enter the password listed in the `Passwords.txt` file.
. Restore BMC network connectivity for the appliance using one of these options: 
