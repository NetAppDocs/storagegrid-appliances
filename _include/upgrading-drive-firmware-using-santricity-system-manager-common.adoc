//This include file is used for the SG5700 and all storage appliances after 11.6. SG6000 uses the cachegroup file.
.Before you begin
...
+
NOTE: WWhile the appliance is in maintenance mode, I/O (input/output operations) activity to the storage controller is stopped to make disruptive storage operations safe.
...
NOTE: Drives belonging to volumes that don't have redundancy must be updated using the offline method. The offline method should be used for any drive associated with flash read cache or any pool or volume group that is currently degraded. 
...
CAUTION: There are two types of drives: SSD and HDD. You must use the offline method to upgrade the firmware on SSDs. You can use either the online or offline method to upgrade the firmware on HDDs.
