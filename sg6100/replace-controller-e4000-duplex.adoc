...
. Open the cover by pressing the blue buttons on the sides of the controller canister to release the cover, and then rotate the cover up and off of the controller canister.
. Determine if the the replacement controller contains a battery and/or a DIMM. If it does, reinstall the controller cover and go to <<step8_replace_controller,Step 8: Replace controller>>. Otherwise go to <<step5_remove_battery, Step 5: Remove the battery>>.
...
. If the replacement controller has pre-installed DIMM, go to <<step7_install_battery,Step 7: Install the battery>>. Otherwise, continue to the next step.
