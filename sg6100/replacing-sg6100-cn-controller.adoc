...
. If the appliance you replaced had hardware drive encryption enabled for the SED drives, you must https://review.docs.netapp.com/us-en/storagegrid-appliances_quartet-idevine-20231016/installconfig/optional-enabling-node-encryption.html#access-an-encrypted-drive[enter the drive encryption passphrase] to access the encrypted drives when the replacement appliance starts for the first time.
