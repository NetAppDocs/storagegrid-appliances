...
summary: 'The SG6160 appliance contains two SSD drives in the SG6100-CN controller. The drives are mirrored using RAID1 for redundancy. If one of the drives fails, you must replace it as soon as possible to ensure redundancy.'
---
...
[.lead]
The SG6160 appliance contains two SSD drives in the SG6100-CN controller. The drives are mirrored using RAID1 for redundancy. If one of the drives fails, you must replace it as soon as possible to ensure redundancy.
...
+
The drive is automatically rebuilt with mirrored data from the working drive. You can check the status of the rebuild by using the Grid Manager. Go to *NODES* > `*Appliance Node*` > *Hardware*. The Storage RAID Mode field contains a "rebuilding" message until the drive is completely rebuilt.
