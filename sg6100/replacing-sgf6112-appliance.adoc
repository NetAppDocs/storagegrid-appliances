...
. If the appliance you replaced had drive encryption enabled for the SED drives, you must link:optional-enabling-node-encryption.html#access-an-encrypted-drive[enter the drive encryption passphrase] to access the encrypted drives when the replacement compute node starts for the first time.
