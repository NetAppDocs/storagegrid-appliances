...
---
= Replace OCP NIC in the SG6100-CN
:icons: font
:imagesdir: ../media/

[.lead]
You might need to replace an OCP Network Interface Card (NIC) in the SG6100-CN if it is not functioning optimally or if it has failed.
...
* You have disconnected all cables.
...
+
image::../media/sg6100_cn_nic_screw.png[Unscrewing NIC faceplate.]
. Carefully remove the card by pulling on the faceplate handle and place it on a flat, anti-static surface.
...
. Align the NIC with the opening in the chassis and carefully push it in until fully secured.
. Tighten the screw on the card.
