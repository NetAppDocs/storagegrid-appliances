...
[.lead]
You might need to replace an external, OCP Network Interface Card (NIC) in the SG6100-CN if it is not functioning optimally or if it has failed.
...
+
CAUTION: Hot-swapping is *not* supported for this procedure. The controller must be disconnected from power even if it remains in the rack.
