 ...
 ** The target node now uses the IP addresses originally assigned to the source node for the Grid, Admin, and Client Networks.
 +
 NOTE:: If you encounter an IP address conflict at the end of the cloning process, make sure that that there were no permanent DHCP leases assigned for any of the source node static MAC addresses of any ports enabled for DHCP on the target node.
