...
---
= How StorageGRID appliance node cloning works
:icons: font
:imagesdir: ../media/
...
* Cloning already in progress
* Wrong network configuration
* Target appliance cannot reach the Primary Admin Node
* Lack of connectivity between the source and target appliances
* Source and target appliances are not compatible
* Source and target appliances are in different maintenance mode states
* StorageGRID Appliance Installer versions on source and target appliances do not match
* For Storage Nodes, the target appliance has incompatible storage or insufficient capacity
* Target appliance is not ready
...
** The number of volumes in a target storage appliance must be greater than or equal to the number of volumes in the source node. The SGF6112 has 12 volumes, and the SGF6212 has 13 volumes. All other storage appliances have 16 volumes.
...
*** You cannot clone a source node with 16 object store volumes (rangedb) to a target storage appliance with fewer object store volumes, even if the target appliance has larger capacity than the source node.
...
// 12.0
Node cloning is not supported from an SG110 source node appliance without node encryption to an SG110 appliance target with node encryption.

=== SG120 appliance cloning information and restrictions

You can clone an SG120 source node appliance to an SG1200 services appliance target to give the Admin Node or Gateway Node greater capability.

// 12.0
Node cloning is not supported from an SG120 source node appliance without node encryption to an SG120 appliance target with node encryption.
...
// 12.0
Node cloning is not supported from an SG1100 source node appliance without node encryption to an SG1100 appliance target with node encryption.

=== SG1200 appliance cloning information and restrictions

You can clone an SG1200 source node appliance to an SG120 services target appliance.

* Cloning the SG1200 to an SG120 services target appliance lets you redeploy the SG1200 for a more demanding application. For example, if an SG1200 source node appliance is being used as an Admin Node and you want to use it as a dedicated load-balancing node.

* Replacing an SG1200 source node appliance with an SG120 services target appliance reduces the maximum speed of the network ports from 100-GbE to 25-GbE.

// 12.0
Node cloning is not supported from an SG1200 source node appliance without node encryption to an SG1200 appliance target with node encryption.
...
* An SG5760 source node appliance configured with DDP or DDP-16 _cannot_ be cloned to an SG6160 or SG6260 storage target appliance with a matching RAID mode.
...
* You can clone an SG6060 source node appliance without any expansion shelves to an SG6160 or SG6260 storage target appliance without any expansion shelves if the drive sizes and RAID modes are the same.

* An SG6060 source node appliance with any number of expansion shelves _cannot_ be cloned to an SG6160 or SG6260 storage target appliance even if the number of expansion shelves, drive sizes, and RAID modes are the same.

// 12.0
=== SGF6112 appliance cloning information and restrictions

Node cloning is not supported from an SGF6112 source node appliance without node encryption to an SGF6112 or SGF6212 appliance target with node encryption.
