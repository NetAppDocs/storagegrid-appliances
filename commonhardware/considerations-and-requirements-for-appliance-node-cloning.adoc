...
---
= Requirements for StorageGRID appliance node cloning
:icons: font
:imagesdir: ../media/
...
* If you are enabling DHCP on any ports of the target node that were configured as static on the source node, make sure that there are no permanent DHCP leases for any of the source node MAC addresses. Failure to do so could cause an IP address conflict at the end of the cloning process.
...
+
// 12.0
The source node and target appliance can have different node-encryption settings with the following exceptions. Node cloning isn't supported from unencrypted-node sources to node-encrypted targets for the following appliances:
+
...
** An SG1200 node appliance without node encryption to an SG1200 appliance with node encryption 
** An SG110 node appliance without node encryption to an SG110 appliance with node encryption
** An SG120 node appliance without node encryption to an SG120 appliance with node encryption
